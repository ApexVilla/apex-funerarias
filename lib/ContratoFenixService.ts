...
import {
  ...
  type ContratoEmpresaJuridica,
} from './contratoEmpresaJuridica';

export interface ContratoFenixData {
  ...
  /** Assinatura eletrônica do cliente (quando já assinou pelo link). */
  assinaturaDigital?: AssinaturaDigitalPdfInfo | null;
  /** Dados jurídicos da unidade emissora (Catalão, Aparecida…). */
  empresaJuridica?: ContratoEmpresaJuridica | null;
}

export const buildContratoFenixPdfBlob = async (data: ContratoFenixData): Promise<Blob> => {
  ...
  y = await drawCapaContratoFenix(doc, W, y, {
    ...
    linhaCertificado: 'Certificado de Adesão Termos e Condições Fênix',
  });
  ...
  if (listaDependentes.length > 0) {
    await drawContratoAnexoDependentes(doc, {
      ...
      dependentes: listaDependentes,
      subtituloPlano: 'Certificado de Adesão Termos e Condições Fênix',
