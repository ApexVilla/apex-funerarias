-- propagate filial_id to monthly installments and backfill existing ones

-- 1. Update fn_gerar_mensalidades to insert filial_id
...
BEGIN
    ...
    FOR i IN 1..p_meses LOOP
        v_codigo_base := 'CR-' || (EXTRACT(EPOCH FROM NOW())::BIGINT + i)::TEXT;
        ...
        INSERT INTO fin_contas_receber (
            ...
            p_assinatura_id,
            'mensalidade',
            'Mensalidade ' || i || '/' || p_meses || ' - ' || COALESCE(v_plano.nome, 'Plano Associativo'),
...
-- 2. Update fn_gerar_mensalidades_com_historico to insert filial_id
...
-- 3. Retro-active backfill for existing monthly accounts receivable
UPDATE public.fin_contas_receber cr
SET filial_id = a.filial_id
FROM public.assinaturas a
WHERE cr.assinatura_id = a.id
  AND cr.filial_id IS NULL
