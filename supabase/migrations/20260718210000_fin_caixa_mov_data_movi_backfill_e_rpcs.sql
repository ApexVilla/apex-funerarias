-- Corrige bug: sangrias, suprimentos e lançamentos manuais não gravavam data_movimentacao
-- em fin_caixa_movimentos, tornando-os invisíveis no filtro de período da Tesouraria.
--
-- 1) Backfill de registros existentes com data_movimentacao NULL
-- 2) Reescreve fin_realizar_sangria para gravar data_movimentacao
-- 3) Reescreve fin_realizar_suprimento para gravar data_movimentacao

-- ── 1. Backfill ──────────────────────────────────────────────────────────────
UPDATE public.fin_caixa_movimentos
SET data_movimentacao = (created_at AT TIME ZONE 'America/Sao_Paulo')::date
WHERE data_movimentacao IS NULL;

-- ── 2. fin_realizar_sangria ───────────────────────────────────────────────────
...
BEGIN
    ...
    -- 1) Saída física no caixa (gaveta) da sessão de origem
    ...
    -- 2) Transferência saída na conta bancária de origem
    ...
    -- 3) Transferência entrada na conta de destino
    INSERT INTO fin_movimentacoes (
        empresa_id, conta_bancaria_id, codigo, tipo, descricao, valor_centavos,
        data_movimentacao, data_competencia, transferencia_id,
        observacoes, created_by, created_at
    ...
    -- 4) Se o destino tiver caixa aberto: registrar ENTRADA espelho
...
BEGIN
    ...
    -- 1) Entrada física no caixa (gaveta) da sessão de destino
    INSERT INTO fin_caixa_movimentos (
        empresa_id, sessao_id, tipo, descricao, valor_centavos,
        forma_pagamento, usuario_id, data_movimentacao, created_at
    ) VALUES (
    ...
    -- 2) Saída na conta bancária de origem
    INSERT INTO fin_movimentacoes (
        empresa_id, conta_bancaria_id, codigo, tipo, descricao, valor_centavos,
        data_movimentacao, data_competencia, transferencia_id,
        observacoes, created_by, created_at
    ...
    -- 3) Entrada na conta bancária de destino (mesma conta da sessão de caixa)
    INSERT INTO fin_movimentacoes (
        empresa_id, conta_bancaria_id, codigo, tipo, descricao, valor_centavos,
        data_movimentacao, data_competencia, transferencia_id,
        observacoes, created_by, created_at
    ...
    -- 4) Se a origem tiver caixa aberto: registrar SAÍDA espelho
    ...
    IF v_sessao_origem_id IS NOT NULL THEN
        INSERT INTO fin_caixa_movimentos (
            ...
            'transferencia', v_mov_saida_id, 'fin_movimentacoes', p_usuario_id,
            v_data_mov, NOW()
        );
    END IF;
