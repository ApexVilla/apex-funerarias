-- Proposta: endereço residencial e cobrança com logradouro, número, bairro, quadra e lote separados.

ALTER TABLE public.propostas_venda
  ADD COLUMN IF NOT EXISTS endereco_logradouro text,
  ADD COLUMN IF NOT EXISTS endereco_numero text,
  ADD COLUMN IF NOT EXISTS endereco_bairro text,
  ADD COLUMN IF NOT EXISTS endereco_quadra text,
  ADD COLUMN IF NOT EXISTS endereco_lote text,
  ADD COLUMN IF NOT EXISTS cobrador_endereco_logradouro text,
  ADD COLUMN IF NOT EXISTS cobrador_endereco_numero text,
  ADD COLUMN IF NOT EXISTS cobrador_endereco_bairro text,
  ADD COLUMN IF NOT EXISTS cobrador_endereco_quadra text,
  ADD COLUMN IF NOT EXISTS cobrador_endereco_lote text;
...
BEGIN
  ...
  FOR v_tentativa IN 1..5 LOOP
    BEGIN
      INSERT INTO public.propostas_venda (
        ...
        NULLIF(trim(p_payload->>'endereco_logradouro'), ''),
        NULLIF(trim(p_payload->>'endereco_numero'), ''),
        NULLIF(trim(p_payload->>'endereco_bairro'), ''),
        NULLIF(trim(p_payload->>'endereco_quadra'), ''),
        NULLIF(trim(p_payload->>'endereco_lote'), ''),
        ...
        NULLIF(trim(p_payload->>'cobrador_endereco_logradouro'), ''),
        NULLIF(trim(p_payload->>'cobrador_endereco_numero'), ''),
        NULLIF(trim(p_payload->>'cobrador_endereco_bairro'), ''),
        NULLIF(trim(p_payload->>'cobrador_endereco_quadra'), ''),
        NULLIF(trim(p_payload->>'cobrador_endereco_lote'), ''),
  ...
  RAISE EXCEPTION 'Não foi possível gerar número da proposta' USING ERRCODE = '23505';
END;
$function$;
