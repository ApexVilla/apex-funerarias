-- Conta corrente na Tesouraria: extrato e saldo final com todas as formas (boleto, PIX, cartão, espécie).
-- A migration 20260722140000 limitou fin_sync_baixas_caixa_sessao só a tipo 'caixa'; esta restaura
-- corrente e usa saldo total (não só espécie) para contas bancárias.

CREATE OR REPLACE FUNCTION public.fin_caixa_saldo_total_sessao(p_sessao_id uuid)
RETURNS bigint
LANGUAGE sql
STABLE
AS $$
...
CREATE OR REPLACE FUNCTION public.fin_caixa_saldo_sistema_sessao(p_sessao_id uuid)
RETURNS bigint
LANGUAGE sql
STABLE
AS $$
    SELECT CASE lower(COALESCE(cb.tipo, ''))
        WHEN 'corrente' THEN public.fin_caixa_saldo_total_sessao(p_sessao_id)
        ELSE public.fin_caixa_saldo_fisico_sessao(p_sessao_id)
    END
    FROM fin_caixa_sessoes s
...
BEGIN
    ...
    IF v_inserted > 0 THEN
        UPDATE fin_caixa_sessoes s
           SET saldo_sistema_centavos = public.fin_caixa_saldo_sistema_sessao(p_sessao_id)
         WHERE s.id = p_sessao_id;
    END IF;
...
-- fin_baixar_conta_receber: sincroniza na conta destino (caixa ou corrente), não só no caixa do operador.
...
GRANT EXECUTE ON FUNCTION public.fin_caixa_saldo_total_sessao(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fin_caixa_saldo_sistema_sessao(uuid) TO authenticated;
...
-- Backfill: baixas em contas corrente/caixa sem movimento na Tesouraria (últimos 90 dias)
DO $backfill$
DECLARE
    r RECORD;
    v_sessao_id uuid;
...
-- Recalcula saldo sistema (físico no caixa, total na corrente)
UPDATE fin_caixa_sessoes s
   SET saldo_sistema_centavos = public.fin_caixa_saldo_sistema_sessao(s.id)
 WHERE EXISTS (
     SELECT 1 FROM fin_contas_bancarias cb
