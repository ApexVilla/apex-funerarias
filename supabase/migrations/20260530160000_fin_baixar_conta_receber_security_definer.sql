-- Migration: Redefinição de fin_baixar_conta_receber e fin_atualizar_vencidos_receber como SECURITY DEFINER
-- Nome do arquivo: 20260530160000_fin_baixar_conta_receber_security_definer.sql

-- 1) Redefine fin_atualizar_vencidos_receber como SECURITY DEFINER
CREATE OR REPLACE FUNCTION public.fin_atualizar_vencidos_receber(p_empresa_id uuid)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Garante que ambos roles possam executar a função
GRANT EXECUTE ON FUNCTION public.fin_atualizar_vencidos_receber(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fin_atualizar_vencidos_receber(uuid) TO anon;


-- 2) Redefine fin_baixar_conta_receber como SECURITY DEFINER
-- Remove assinaturas antigas se existirem
DROP FUNCTION IF EXISTS public.fin_baixar_conta_receber(uuid, bigint, uuid, uuid, bigint, text, uuid, date);
DROP FUNCTION IF EXISTS public.fin_baixar_conta_receber(uuid, bigint, uuid, uuid, bigint, text, uuid, date, bigint, bigint);
...
GRANT EXECUTE ON FUNCTION public.fin_baixar_conta_receber(uuid, bigint, uuid, uuid, bigint, bigint, bigint, text, date, uuid) TO anon;
