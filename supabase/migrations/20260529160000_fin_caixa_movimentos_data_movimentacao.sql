-- Data de referência do movimento de caixa (= dia da baixa), para Tesouraria e relatórios.
...
UPDATE public.fin_caixa_movimentos m
   SET data_movimentacao = COALESCE(
     m.data_movimentacao,
     (SELECT s.data_abertura::date FROM public.fin_caixa_sessoes s WHERE s.id = m.sessao_id),
     m.created_at::date
   )
 WHERE m.data_movimentacao IS NULL;

ALTER TABLE public.fin_caixa_movimentos
  ALTER COLUMN data_movimentacao SET DEFAULT CURRENT_DATE;

CREATE INDEX IF NOT EXISTS idx_fin_caixa_movimentos_data_mov
  ON public.fin_caixa_movimentos (empresa_id, data_movimentacao DESC);

-- fin_baixar_conta_receber: grava data_movimentacao no caixa do dia do pagamento
...
BEGIN
    ...
    v_ts_mov := (v_data::timestamp + time '12:00:00') AT TIME ZONE 'America/Sao_Paulo';

    SELECT * INTO v_cr FROM fin_contas_receber WHERE id = p_conta_receber_id FOR UPDATE;
    IF NOT FOUND THEN RAISE EXCEPTION 'Titulo a receber % nao encontrado', p_conta_receber_id; END IF;
    IF v_cr.status IN ('pago','cancelado') THEN RAISE EXCEPTION 'Titulo com status % nao pode ser baixado', v_cr.status; END IF;

    INSERT INTO fin_contas_receber_baixas (
        empresa_id, conta_receber_id, valor_pago_centavos, valor_desconto_centavos,
        valor_juros_centavos, valor_multa_centavos, forma_pagamento_id, conta_bancaria_id,
        observacoes, tipo, created_by
    ) VALUES (
        v_cr.empresa_id, p_conta_receber_id, p_valor_pago_centavos, p_valor_desconto_centavos,
        p_valor_juros_centavos, p_valor_multa_centavos, p_forma_pagamento_id, p_conta_bancaria_id,
        p_observacoes,
        CASE WHEN p_valor_pago_centavos >= (v_cr.valor_original_centavos - v_cr.valor_pago_centavos)
             THEN 'normal' ELSE 'parcial' END,
        v_uid
    ...
    v_total_devido := v_cr.valor_original_centavos + v_cr.valor_juros_centavos
                    + v_cr.valor_multa_centavos - v_cr.valor_desconto_centavos;

    IF v_cr.valor_pago_centavos >= v_total_devido THEN v_novo_status := 'pago';
    ELSE v_novo_status := 'pago_parcial'; END IF;
    ...
    v_conta_origem_id := COALESCE(
        p_conta_bancaria_id,
        (SELECT id FROM fin_contas_bancarias WHERE empresa_id = v_cr.empresa_id AND principal = true LIMIT 1)
    );
    ...
    INSERT INTO fin_movimentacoes (
        empresa_id, filial_id, codigo, conta_bancaria_id, plano_conta_id, centro_custo_id,
        tipo, descricao, valor_centavos, data_movimentacao, data_competencia,
        conta_receber_id, conta_receber_baixa_id, created_by
    ) VALUES (
        v_cr.empresa_id, v_cr.filial_id,
        'MOV-' || to_char(now(), 'YYYYMMDD-HH24MISS-US'),
        v_conta_origem_id, v_cr.plano_conta_id, v_cr.centro_custo_id,
        'receita',
        'Recebimento: ' || v_cr.codigo || COALESCE(' - ' || v_cr.descricao, ''),
        p_valor_pago_centavos, v_data, v_cr.data_competencia,
        p_conta_receber_id, v_baixa_id, v_uid
    );

    v_forma_caixa := COALESCE(
        (SELECT CASE lower(trim(COALESCE(tipo, nome, 'dinheiro')))
            WHEN 'dinheiro' THEN 'especie' WHEN 'espécie' THEN 'especie' WHEN 'especie' THEN 'especie'
            WHEN 'pix' THEN 'pix' ELSE lower(trim(COALESCE(tipo, nome, 'dinheiro'))) END
    ...
    IF v_conta_origem_id IS NOT NULL THEN
        ...
        IF v_sessao_id IS NOT NULL THEN
            INSERT INTO fin_caixa_movimentos (
                ...
                'Recebimento ' || v_cr.codigo || COALESCE(' - ' || v_cr.descricao, ''),
                p_valor_pago_centavos, p_conta_receber_id, 'fin_contas_receber',
                v_forma_caixa, v_uid, v_data, v_ts_mov
            );
        END IF;
    END IF;
