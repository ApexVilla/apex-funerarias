-- Lista cobradores ativos da unidade (grupo econômico + filtro por nome da unidade).
-- Contorna RLS de filiais (só empresa do cadastro) e consultas só na empresa do topo.

CREATE OR REPLACE FUNCTION public.fn_normalizar_unidade_txt(p text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
REVOKE ALL ON FUNCTION public.fn_normalizar_unidade_txt(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_normalizar_unidade_txt(text) TO authenticated;

REVOKE ALL ON FUNCTION public.fn_cobradores_ativos_para_unidade(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_cobradores_ativos_para_unidade(text) TO authenticated;

COMMENT ON FUNCTION public.fn_cobradores_ativos_para_unidade(text) IS
  'Cobradores ativos do grupo econômico do usuário, filtrados por token da unidade (ex.: Catalao).';
