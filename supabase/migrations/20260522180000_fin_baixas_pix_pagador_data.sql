-- Baixas: data da baixa + identificação do pagador PIX (terceiro vs titular)

ALTER TABLE public.fin_contas_receber_baixas
  ADD COLUMN IF NOT EXISTS data_baixa date,
  ADD COLUMN IF NOT EXISTS pix_mesmo_pagador boolean,
  ADD COLUMN IF NOT EXISTS pix_nome_pagador text;

COMMENT ON COLUMN public.fin_contas_receber_baixas.data_baixa IS
  'Data em que o recebimento foi registrado (baixa).';
COMMENT ON COLUMN public.fin_contas_receber_baixas.pix_mesmo_pagador IS
  'TRUE quando o PIX foi pago pelo próprio cliente/titular; FALSE quando pagador é terceiro.';
COMMENT ON COLUMN public.fin_contas_receber_baixas.pix_nome_pagador IS
  'Nome do pagador no comprovante PIX, quando pix_mesmo_pagador = false.';

ALTER TABLE public.cob_recebimentos_campo
  ADD COLUMN IF NOT EXISTS pix_mesmo_pagador boolean,
  ADD COLUMN IF NOT EXISTS pix_nome_pagador text;

COMMENT ON COLUMN public.cob_recebimentos_campo.pix_mesmo_pagador IS
  'TRUE quando o PIX foi pago pelo próprio cliente; FALSE quando terceiro pagou.';
COMMENT ON COLUMN public.cob_recebimentos_campo.pix_nome_pagador IS
  'Nome do pagador no comprovante PIX (cobrança em campo).';

-- Backfill data_baixa a partir de created_at onde ainda nulo
UPDATE public.fin_contas_receber_baixas
   SET data_baixa = (created_at AT TIME ZONE 'America/Sao_Paulo')::date
 WHERE data_baixa IS NULL;

DROP FUNCTION IF EXISTS public.fin_baixar_conta_receber(uuid, bigint, uuid, uuid, bigint, bigint, bigint, text, date, uuid);
...
BEGIN
    ...
    INSERT INTO fin_contas_receber_baixas (
        ...
        tipo, created_by,
        data_baixa, pix_mesmo_pagador, pix_nome_pagador
    ) VALUES (
