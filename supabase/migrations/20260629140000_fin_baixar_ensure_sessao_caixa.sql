-- Garante sessão de caixa na data da baixa antes de sincronizar movimentos (evita baixas órfãs na Tesouraria).
...
-- Re-sincroniza baixas recentes sem movimento de caixa
DO $backfill$
DECLARE
    r RECORD;
    v_sessao_id uuid;
...
BEGIN
    FOR r IN
        ...
        JOIN fin_contas_bancarias cb_op
         ...
         AND (
             cardinality(COALESCE(cb_op.autorizados_operacao, ARRAY[]::uuid[])) = 0
             OR b.created_by = ANY(cb_op.autorizados_operacao)
         )
        WHERE COALESCE(b.estornada, false) = false
          ...
          AND COALESCE(b.data_baixa, (b.created_at AT TIME ZONE 'America/Sao_Paulo')::date)
              >= (CURRENT_TIMESTAMP AT TIME ZONE 'America/Sao_Paulo')::date - 45
          AND NOT EXISTS (
    ...
    LOOP
        SELECT s.id INTO v_sessao_id
         ...
         ORDER BY s.data_abertura DESC         LIMIT 1;
    ...
    RAISE NOTICE 'backfill baixas caixa: % movimento(s)', total;
END;
$backfill$;
